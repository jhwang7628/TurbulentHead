...
void Mesh::ExtractSurface() 
{
    ...
    int count = 0;
    for (unsigned int i=0; i<this->meshSurface->vertlist.size(); i++)
    {
    ...
    for (unsigned int i=0; i<this->meshSurface->trilist.size(); i++)
    {
        int i1, i2, i3; 
...
void Mesh::assignK()
{
    ...
    //if (!in) 
    //{
    //    cerr << "Cannot read file " << in_file << ". Exiting" << endl;
    //    exit(1);
    //}
    //
    //
    ...
    pFile = fopen("out/computedK.txt", "w");

    if (pFile == NULL) 
    {
        cerr << "Cannot open file computedK.txt " << ". Exiting" << endl;
        exit(1);                                                          
    }
    ...
    //for (int j=0; j<3000; j++)
    {
        ...
        for (unsigned int i=0; i<this->meshSurface->vertlist.size(); i++)
        {
            if (this->meshSurface->vertlist[i].isOnSurface)
            {
                if ((this->meshSurface->vertlist[i].position - fluentCoord).normSq() < 1e-12)
                {
                    bestInd = i; 
                    fprintf(pFile, "%.9E %.9E %.9E ", fluentCoord.x,fluentCoord.y,fluentCoord.z);
                    fprintf(pFile, "%.9f\n", this->meshSurface->vertlist[i].curvature);
                }
