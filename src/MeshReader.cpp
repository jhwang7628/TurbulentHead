...
{ 
    ...
    cout << "count_onsurface vertices = " << count_onsurface + 1 << endl;
...
void Mesh::ExtractSurface() 
{
    ...
    {
        if (this->meshSurface->vertlist[i].isOnSurface) 
        {
            this->meshSurface->vertlist[i].correspondVert = count;
            vert newVert; 
            newVert.setEqual(this->meshSurface->vertlist[i]);
            this->extractedSurface->vertlist.push_back(newVert);
            //cout << "after push_back, current number of vertex in extractedSurface = " << extractedSurface->vertlist.size() << endl;
            count ++; 
        }
    }
    //cout << "total number of vertex on surface = " << count << endl;
    //cout << "total number of vertex in extractedSurface = " << extractedSurface->vertlist.size() << endl;

    for (uint i=0; i<this->meshSurface->trilist.size(); i++)
    {
        int i1, i2, i3; 
        i1 = this->meshSurface->trilist[i].index.x;
        i2 = this->meshSurface->trilist[i].index.y;
        i3 = this->meshSurface->trilist[i].index.z;

        tri newTri; 

        newTri.setEqual(this->meshSurface->trilist[i]);

        newTri.index.x = this->meshSurface->vertlist[i1].correspondVert;
        newTri.index.y = this->meshSurface->vertlist[i2].correspondVert;
        newTri.index.z = this->meshSurface->vertlist[i3].correspondVert;
        newTri.ObtuseIndex = this->meshSurface->vertlist[newTri.ObtuseIndex].correspondVert;


        neighbor newNeighbor;
        newNeighbor.neighborTri = newTri;
        extractedSurface->vertlist[newTri.index.x].N1neighbors.push_back(newNeighbor); 
        extractedSurface->vertlist[newTri.index.y].N1neighbors.push_back(newNeighbor); 
        extractedSurface->vertlist[newTri.index.z].N1neighbors.push_back(newNeighbor); 

        this->extractedSurface->trilist.push_back(newTri);
    }
}
...
void surface::print2WaveSolver(string filename)
{

    cout << "generating gpu wavesolver input" << endl;
