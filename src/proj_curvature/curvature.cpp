...
void surface::computeK()
{
   ...
   for (int i=0; i<Nvert; i++)
   {
      if (vertlist[i].isOnSurface) 
      {
          ...
          double sum_N1AV = 0;
          for (unsigned long j=0; j<neighborVert.size() && (neighborVert[j]!=i); j++) 
          {
      ...
      if (vertlist[i].isOnSurface)
      {
         ...
         //cout << "==========vertex " << i << endl;
          ...
          for (int j=0; j<N_N1neighbors; j++) 
          {
             ...
             {
          //cout << "A_Voronoi = " << vertlist[i].A_Voronoi << endl;
                vertlist[i].A_mixed += vertlist[i].A_Voronoi; 
             }
             else 
             {
          //cout << "Area = " << trilist[j].area << endl;
          ...
          vertlist[i].curvature = 1.0/2.0/vertlist[i].A_mixed; 
      fA << vertlist[i].curvature << endl;
      }
